...
### 已知局限：`/readyz` 检测不到"线程活着但 WS 重连一直失败"

`/readyz` 现在只看 `lark-ws-client` 这个后台线程是否 `is_alive()`。如果 SDK 内部的 WS
连接断了之后陷在重连失败循环里（鉴权过期、网络分区、飞书侧拒绝），线程仍然是 alive 的，
`/readyz` 还会返回 200，但实际收不到任何事件——也就是"假活"。

这是测试环境答疑场景下的有意取舍：要做真正的链路健康检查需要拿到 SDK 的连接/心跳状态
（目前 lark-oapi 没暴露稳定回调），实现成本不划算。生产环境如果需要兜住这种场景，可以
对 `last_event_age_seconds` 做趋势告警（比如工作时间内 > 2h 没事件就人工查一下），但**不要**
拿它做自动重启依据，否则夜里没人提问就会被反复杀。
