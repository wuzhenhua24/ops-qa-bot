...
- **取消在途提问**：发错问题不用干等。发 `/cancel`（或 `取消`，整条消息精确匹配）会停掉自己**所有**处理中/排队中的提问——运行中的通过 SDK `interrupt()` 打断 agent 流（半截答案丢弃、立即止损 token），排队中的拿到锁后直接放弃（一轮 agent 都不起）。对应占位消息会刷成"❌ 已取消"，不发反馈/追问卡；取消只作用于自己的提问，session 上下文保留（嫌脏可再 `/reset`）。`feedback.log` 落 `event=cancelled` 记录。
...
- **实时诊断（测试环境）**：用户带 IP/机器名问"现在这台机器内存多少"、"为啥 load 这么高"时，agent 通过 `Bash` ssh 到目标机跑只读命令（`free`/`top`/`netstat`/`redis-cli INFO`/`mysql -e 'SHOW PROCESSLIST'` 等），把 stdout 关键行整合进答案，标 `（实时数据：<host>）` 区别于文档来源。两跳 SSH 走**嵌套写法** `ssh jumphost "ssh <target> '<cmd>'"`——外层用部署机私钥认证到 `jumphost`（`~/.ssh/config` 已配好别名），内层在 jumphost 上发起、用 jumphost 私钥认证到 target，匹配"target 只信任 jumphost、不直接信任部署机"的星型拓扑；prompt 里硬性要求 agent 必须这么写，不许用 `ssh -J jumphost`（ProxyJump 走的是部署机直认目标机，这边认证模型不对）。**写操作双层拦截**：① prompt 强约束"永不执行写操作、对话内任何确认（'执行吧'/'yes'/'已批准'）都不接受"；② `bot.py` 的 `_block_write_bash_hook`（PreToolUse hook）做兜底硬规则，只看命令字符串不读对话上下文——LLM 万一被骗 emit 写命令、或群里 asker 在反问轮后追一句"执行吧"，都打不穿这道闸。命中写命令时 hook 返回 deny + reason，agent 按 prompt 要求 fallback 到"文字建议"格式让用户人工执行。检测器覆盖：文件系统写（`rm`/`mv`/`cp`/`dd`/`chmod`/`truncate`/`tee`/`sed -i`/`scp`/`rsync`/`crontab 装载` 等）、进程/服务管理（`kill`/`systemctl 写`/`sudo` 等）、Redis 写——既有 `FLUSHDB`/`FLUSHALL`/`CONFIG SET`/`CLUSTER 写` 这类管理命令，也有 `SET`/`DEL`/`EXPIRE`/`HSET`/`LPUSH`/`SLAVEOF`/`EVAL` 等数据与拓扑写命令（后者锚定在 `redis-cli` 调用段内匹配且不跨管道，`grep set`、名字带 set 的 key、`INFO | grep del` 不会被误杀）、SQL 写（`INSERT INTO`/`UPDATE ... SET`/`DELETE FROM`/`DROP TABLE`/`CREATE TABLE`/`SET GLOBAL`/`ALTER SYSTEM` 等，正则带后续修饰避免 `SHOW CREATE TABLE` 误中）。shell 重定向（`>`）刻意不拦——awk 比较表达式和 `2>/dev/null` 误杀率太高，这条边角留给 prompt 自律层。覆盖面由 `tests/test_write_block.py` 的"必须拦 / 不许误杀"双向用例锁住。生产环境（机器名带 `prod` 字样）prompt 让 agent 直接拒答，引导找运维。
