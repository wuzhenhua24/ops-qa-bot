...
4. **不要在正文里直接写** `@张三`、`@ou_xxx`、`(open_id: ou_xxx)` 或 markdown 链接——这些都没用，`open_id` 是系统内部标识，写进正文用户只会看到一串没意义的 `ou_xxx` 乱码。系统会**只**根据 `<<ESCALATE:...>>` 标记来渲染 @ 提醒。正文里提负责人就**写姓名**（如"建议联系网关负责人张三确认"），open_id 只放进 `<<ESCALATE:ou_xxx:dir>>` 标记里。
