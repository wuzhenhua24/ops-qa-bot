...
_FEISHU_DOC_TOOL_LINE = (
    "\n- `mcp__feishu_doc__ask_feishu_doc(docs, question)`：读取组件 `feishu_docs.md`"
    "里登记的**飞书云文档**并基于这些文档回答（部分组件知识写在飞书里、未同步到"
    "本地 md）。`docs` 是 URL 数组，可一次传多条；URL 必须是 `feishu_docs.md` 已登记的，"
    "不要凭空构造；返回的是协作机器人 lark-copilot 基于这些文档给出的答案，可以"
    "引用、二次加工，但**不要**伪装成你自己读到的（见下方「飞书文档协作」节）。"
)
...
2. **优先级**：本地 md 已经能完整回答的，**不需要**再调飞书工具（飞书慢、飞书结果
   ...
   两边都用、合并答。
3. **调用方式**：`ask_feishu_doc(docs=[<feishu_docs.md 里登记的完整 URL>, ...],
   question=<针对这些文档的具体问题>)`。
   - `docs` 是 **URL 数组**：命中多条相关条目时**一次性把它们全传进去**，由协作
     机器人合并阅读；**不要**拆成多次调用（飞书每次调用 10–30s，多次串行很慢）
   - 每个 URL **只能用 `feishu_docs.md` 里登记的**，不要凭空构造、不要改写参数、
     不要把整篇 `feishu_docs.md` 当一个 URL 传
   - `question` 提炼成一句"针对这些文档想知道什么"（如"OOM 排查的推荐步骤是
     什么"），不要把用户原话整段塞进去；问题面向全部 docs 共同回答，不必逐文档拆
4. **结果当作另一个 agent 的回答**：工具返回的是 lark-copilot 基于这些飞书文档
   作答的成品，不是原文摘录。可以引用、和本地文档结论合并、按格式重排，但**不要**
   说"我读了这些文档" —— 你没读，是协作机器人读的。
5. **来源标注**：飞书文档引用用以下格式（和本地 md 的"（来源：xxx.md）"区分开），
   多篇文档时分别列出：
   `（来源：飞书文档《<标题>》<URL>，由协作机器人协助读取）`
6. **工具失败时降级**：返回里出现"协作机器人未返回答案"、"超时"、"未就绪"等字样
   时，**不要重试同一调用**——把这批飞书文档当作暂时不可用，仅用本地 md 部分作答；
   如果飞书文档是唯一来源、本地 md 没覆盖，回复"飞书文档当前读不到，麻烦打开
   下列 URL 自查或联系组件负责人"并把所有相关 URL 列给用户，再按 INDEX.md
   决定是否走 `<<ESCALATE:...>>`。

## 限制

- 这条工具是慢操作（拉文档 + 协作机器人推理可能 10–30s）。**优先一次调用、传多条
  URL** 覆盖问题需要的所有文档；只有发现第一次返回明显不够、需要补别的文档时才
  追加第二次。一次回答里**最多调两次**——超过基本是 agent 在乱试。
